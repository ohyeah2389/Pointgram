...
try:
    ...
    GLTF2 = None  # Define GLTF2 as None for if import fails
...
# Y_glTF =  Z_OpenCV
# Z_glTF = -Y_OpenCV
OPENCV_TO_GLTF_ROT = np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]], dtype=np.float32)

# Camera local coordinate transformation (OpenCV camera -> glTF camera)
# X_glTF =  X_OpenCV
# Y_glTF = -Y_OpenCV
# Z_glTF = -Z_OpenCV
CAMERA_CONVENTION_ROT = np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]], dtype=np.float32)
...
def export_scene_to_gltf(
    ...
    Args:
        ...
        point_set_names: Dictionary mapping point set ID (int) to custom name (str).
    ...
    if not PYGLTFLIB_AVAILABLE:
        return (
            False,
            "pygltflib library is required but not found. Install with 'pip install pygltflib'.",
        )
    ...
    if missing_keys:
        return (
            False,
            f"Results dictionary is missing required keys: {', '.join(missing_keys)}",
        )

    if not isinstance(results.get("intrinsics"), dict):
        return False, "Results 'intrinsics' dictionary is missing or invalid."

    if not isinstance(point_set_names, dict):
        return False, "Invalid 'point_set_names' dictionary provided."
    ...
    try:
        ...
        if points3d_opencv.size > 0:
            if points3d_opencv.ndim != 2 or points3d_opencv.shape[1] != 3:
                return (
                    False,
                    f"Invalid shape for points_3d numpy array: {points3d_opencv.shape}. Expected (N, 3).",
                )
        ...
        gltf = GLTF2()
        gltf.asset = Asset(version="2.0", generator=generator_name)
        gltf.scene = 0
        scene = Scene(nodes=[])
        gltf.scenes.append(scene)

        img_idx_to_gltf_cam_idx: Dict[int, int] = (
            {}
        )
        znear = 0.01
        zfar = 1000.0
        ...
        for img_idx in registered_indices_int:
            if img_idx not in poses:
                print(
                    f"Warning: Pose missing for registered camera index {img_idx}. Skipping camera definition."
                )
                continue
            ...
            if intrinsic_data and "K" in intrinsic_data:
                ...
                print(
                    f"Warning: Missing intrinsic data or 'K' matrix for camera index {img_idx}. Skipping camera definition."
                )
                continue
            ...
            if K_matrix is None or K_matrix.shape != (3, 3):
                print(
                    f"Warning: Invalid K matrix shape for camera index {img_idx}. Skipping camera definition."
                )
                continue
            if np.isnan(K_matrix).any() or np.isinf(K_matrix).any():
                print(
                    f"Warning: NaN or Inf found in K matrix for camera index {img_idx}. Skipping camera definition."
                )
                continue
            if dims is None or len(dims) != 2 or dims[0] <= 0 or dims[1] <= 0:
                print(
                    f"Warning: Invalid or missing dimensions for camera index {img_idx}. Skipping camera definition."
                )
                continue
            ...
            if 0 <= img_idx < len(image_paths):
                try:
                    cam_def_name = f"{os.path.splitext(os.path.basename(image_paths[img_idx]))[0]}_Def"
                except Exception:
                    pass
        ...
        num_exported_cameras = 0
        exported_cam_indices = []
        for img_idx in registered_indices_int:
            ...
            if R_c2w_cv.shape != (3, 3):
                print(
                    f"Warning: Invalid R matrix shape {R_c2w_cv.shape} for camera {img_idx}. Skipping node."
                )
                continue
            if t_c2w_cv.shape != (3,):
                print(
                    f"Warning: Invalid t vector shape {t_c2w_cv.shape} for camera {img_idx}. Skipping node."
                )
                continue
            if (
                np.isnan(R_c2w_cv).any()
                or np.isinf(R_c2w_cv).any()
                or np.isnan(t_c2w_cv).any()
                or np.isinf(t_c2w_cv).any()
            ):
                print(f"Warning: NaN/Inf in pose for camera {img_idx}. Skipping node.")
                continue
            ...
            try:
                ...
                if (
                    np.isnan(T_cam_to_world_gltf).any()
                    or np.isinf(T_cam_to_world_gltf).any()
                ):
                    print(
                        f"Warning: NaN/Inf in final transform matrix for camera {img_idx}. Skipping node."
                    )
                    continue
                ...
                if 0 <= img_idx < len(image_paths):
                    try:
                        cam_node_name = os.path.splitext(
                            os.path.basename(image_paths[img_idx])
                        )[0]
                    except Exception:
                        pass
                ...
                node = Node(
                    name=cam_node_name, camera=gltf_camera_index, matrix=node_matrix
                )
                gltf.nodes.append(node)
                scene.nodes.append(len(gltf.nodes) - 1)
                num_exported_cameras += 1
                exported_cam_indices.append(img_idx)
                ...
                print(
                    f"Warning: Skipping camera node creation for index '{img_idx}' due to error: {cam_err}"
                )
                continue

        # Create Empty Nodes for Points
        num_exported_points = 0
        exported_point_indices = []
        exported_point_names = []
        if points3d_gltf.size > 0:
            if len(point_ids) != len(points3d_gltf):
                print(
                    f"Warning: Mismatch between number of point IDs ({len(point_ids)}) and 3D points ({len(points3d_gltf)}). Point names might be incorrect."
                )
                # Fallback: Use sequential index if ID mapping is broken
                point_ids_to_use = [None] * len(points3d_gltf)
            else:
                point_ids_to_use = point_ids

            for i, point_gltf in enumerate(points3d_gltf):
                if np.isnan(point_gltf).any() or np.isinf(point_gltf).any():
                    continue

                original_set_id = point_ids_to_use[
                    i
                ]  # Get the original set ID for this point
                node_name = f"Point_{original_set_id}"

                if original_set_id is not None:
                    # Use custom name if available, otherwise keep default
                    custom_name = point_set_names.get(original_set_id)
                    if custom_name:
                        node_name = custom_name  # Use the custom name directly
                    exported_point_indices.append(original_set_id)
                else:
                    # If original_set_id is None (mapping failed?), use index fallback
                    node_name = f"Point_Unknown_{i}"
                    exported_point_indices.append(
                        f"Unknown_{i}"
                    )

                node = Node(name=node_name, translation=point_gltf.tolist())
                gltf.nodes.append(node)
                scene.nodes.append(len(gltf.nodes) - 1)
                num_exported_points += 1
                exported_point_names.append(node_name)

        # Summary and Save
        point_summary = (
            f"{num_exported_points} points exported"
            if num_exported_points == 0
            else f"{num_exported_points} points exported"
        )

        export_summary = (
            f"Exported {len(gltf.cameras)} camera definitions, {num_exported_cameras} camera nodes"
            f"{point_summary}"
        )
        ...
        if num_exported_cameras == 0 and num_exported_points == 0:
            ...
            return (
                False,
                "Export failed: No valid cameras or points could be processed.",
            )
        ...
        return False, "Export failed: pygltflib not found."
    except FileNotFoundError:
        return (
            False,
            f"Export failed: Directory for '{filename}' does not exist or insufficient permissions.",
        )
    except PermissionError:
        return False, f"Export failed: Permission denied writing to '{filename}'."
    except Exception as e:
        import traceback

        print("--- GLTF Export Error ---")
        traceback.print_exc()
        print("--- End Traceback ---")
        return False, f"Export failed: An unexpected error occurred: {e}"
